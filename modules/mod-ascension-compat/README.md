...
## Compatibility work tracking

See the [compatibility tracker](docs/compatibility-tracker.md) for donor-data coverage, expected benefits,
prepared fixes, blocked items and acceptance checks. The linked JSON inventory can be refreshed with the
read-only donor audit script; it is not an import manifest.

The tracker also records the adapted CoA Repack Update3 source fixes and their two world migrations.
They require rebuilding this fork and running its normal database updater; the repack executable and
installer are not used by this source port.
