...
public:
  ...
  bool InitializeLiveBaseline(Player* player)
  {
    ...
    for (auto const& entry : AscensionLiveBaseline::Skills)
    {
      ...
      if (!skill)
        return false;
      // The observed baseline sampled one race per class. Languages belong to
      // that race, not to every character of the sampled class.
      if (skill->categoryId == SKILL_CATEGORY_LANGUAGES &&
          !GetSkillRaceClassInfo(entry.SkillId, player->getRace(), player->getClass()))
        continue;
  ...
  bool RepairStarterKit(Player *player, bool force) {
    ...
    void InitializeStarterActions(Player* player)
    {
        constexpr char setting[] = "core.ascension_starter_actions";
        if (player->GetPlayerSetting(setting, 0).value ||
            (!player->HasAtLoginFlag(AT_LOGIN_FIRST) && player->GetLevel() != 1))
            return;

        // Only repair an existing level-one layout if it is empty/attack-only.
        // Mark the attempt once so deliberately clearing the bar stays cleared.
        player->UpdatePlayerSetting(setting, 0, 1);
        bool const firstLogin = player->HasAtLoginFlag(AT_LOGIN_FIRST);
        std::unordered_set<uint32> placed;
        for (uint8 slot = 0; slot < MAX_ACTION_BUTTONS; ++slot)
        {
            if (ActionButton const* button = player->GetActionButton(slot))
            {
                if (!firstLogin && (button->GetType() != ACTION_BUTTON_SPELL || button->GetAction() != 6603))
                    return;
                if (button->GetType() == ACTION_BUTTON_SPELL)
                    placed.insert(button->GetAction());
            }
        }

        uint8 slot = 0;
        uint32 added = 0;
        auto place = [player, &placed, &slot, &added](uint32 spellId)
        {
            SpellInfo const* spell = sSpellMgr->GetSpellInfo(spellId);
            if (!spell || !player->HasSpell(spellId) || spell->IsPassive() ||
                spell->IsProfessionOrRiding() || placed.count(spellId))
                return;
            while (slot < 12 && player->GetActionButton(slot))
                ++slot;
            if (slot < 12 && player->addActionButton(slot, spellId, ACTION_BUTTON_SPELL))
            {
                placed.insert(spellId);
                ++slot;
                ++added;
            }
        };
        ...
        for (auto const& entry : AscensionCompatData::ClassSpells)
        {
            if (entry.ClassId == player->getClass() && entry.RequiredLevel == 1 &&
                entry.SpellId != 2764 && entry.SpellId != 3018 && entry.SpellId != 5019)
                place(entry.SpellId);
        }
        if (added)
        {
            player->SendActionButtons(1);
            LOG_INFO("module.ascension_compat", "Initialized {} starter action buttons for {} (class {})",
                added, player->GetName(), uint32(player->getClass()));
        }
    }
...
public:
  ...
  void OnPlayerLogin(Player *player) {
    // Collections, wardrobe and account companions are client features; playerbots have no client.
